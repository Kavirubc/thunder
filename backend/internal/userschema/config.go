...
// Store mode constants for user schema service.

// getUserSchemaStoreMode determines the store mode for user schemas.
//
// Resolution order:
//  1. If UserSchema.Store is explicitly configured, use it
...
func getUserSchemaStoreMode() serverconst.StoreMode {
	cfg := config.GetServerRuntime().Config
	// Check if service-level configuration is explicitly set
	if cfg.UserSchema.Store != "" {
		mode := serverconst.StoreMode(strings.ToLower(strings.TrimSpace(cfg.UserSchema.Store)))
		...
		default:
			msg := fmt.Sprintf(
				"Invalid user schema store mode: %s, falling back to global declarative resources setting", mode)
